...
export class ListComponent implements OnInit, AfterViewInit{
  ...
  getFeedDetails(){
    ...
    .subscribe((res:any) => {
      if(res?.data){
        this.feedDetails = res?.data?.getAllFeedDetails;
        this.displayFeed();
      }
      else if(res.loading){
        console.log("Loading...");
  ...
  renderContent(){
    ...
    // the array is frozen so make a copy and then sort it 
    this.objList = this.objList.slice().sort((a:Detail, b:Detail) => {
  ...
  displayFeed(){ 
    ...
    if(diff1.length){
      diff1.forEach((ele: any) => {
        this.api.deleteFeedDetail(ele._id)
        .subscribe(()=>{});
      })
    }
